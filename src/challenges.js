...
function multiplyBy(numbers, numberMultiply) {
  ...
  numbers.forEach((multi) => {
    let result = multi * numberMultiply;
...
function filterOut(original, toRemove) {
  ...
  original.forEach((equals) => {
    if (!toRemove.includes(equals)) {
      copyArray.push(equals);
    }
  });
  return copyArray;
}
...
function uniquifyArray(duplicateWords) {
  ...
  duplicateWords.forEach((equals) => {
    if (!copyArray.includes(equals)) {
      copyArray.push(equals);
    }
  });
  return copyArray;
